...
!function() {
  ...
  $(document).ready(async function() {
    ...
    $("#downloadLogsText").click(async function(e) {
      e.preventDefault();
      let date = new Date();
      let logsformatted = formatLogs($('#logsVerbosity').val());
    ...
    $("#downloadLogsHTML").click(async function(e) {
      e.preventDefault();
      let style = '.sentImg>video.video_note{border-radius:50%;min-width:60%;min-height:60%;}.sentImg,.sentImg>img{display:inline-block;max-width:100%;max-height:30%;text-align:center}html,body{background-color:#000;font-family: Roboto,Montserrat,Lato,sans-serif;padding:15px;}a{text-decoration:none;color:#039be5;-webkit-tap-highlight-color:transparent;}.yellow-text{color:#ffeb3b}.blue-text{color:#2196F3}.red-text{color:#F44336}.green-text{color:#4CAF50}.white-text{color:#fff}';
      let html = '<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, shrink-to-fit=no"><meta name="generator" content="EasyJSBot Log"><title>EasyJSBot Log {0}</title><style>{1}</style></head><body>{2}</body></html>'
      let date = new Date();
      let day = date.getDay();
      ...
      let a = $("<a>")
                      .prop('href', 'data:application/octet-stream;charset=utf-8,'+encodeURIComponent(html.format(formatted, style, logsformatted)))
    ...
    if ("botSettings" in cookies) {
      ...
      if ("selectedChatId" in bSettings && bSettings["selectedChatId"] != 0) {
        setTimeout(async function() {
          ...
          } else log(l["autoSelectCID1"] + knownChatIDs[selectedChatId] + l["autoSelectCID2"], "waiting");
        }, 0);
      }
    }
    M.updateTextFields();
  ...
  async function request(
    ...
    method !== "getUpdates" && log("Doing a " + method + ' method request with POST args "' + JSON.stringify(args) + '"...', 'debug');
    $.ajax({
